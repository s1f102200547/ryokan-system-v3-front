...
export async function PATCH(
  ...
  try {
    ...
    if (e instanceof InfraError) {
      ...
      logger.error('キャンセル InfraError', { infraErrorCode: e.code, message: e.message })
      notifySlackFireAndForget(`[ALERT] キャンセルエラー(${e.code}): ${e.message}`)
      return NextResponse.json({ error: 'internal server error' }, { status })
    }
    logger.error('キャンセル 想定外エラー', { message: String(e) })
    notifySlackFireAndForget(`[ALERT] キャンセルで想定外エラー: ${String(e)}`)
