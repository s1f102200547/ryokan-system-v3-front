...
## セッション切れの時エラー通知だけでlogin画面遷移されない

## モーダル内で値を変更->モーダルを閉じる->モーダルを再度開ける->変更がUI上で反映されてない->リロード->モーダルを開ける->変更が反映されてる(debounce0.5秒が経つ前にモーダルを閉じるとその直後のdb状態でdb再取得(更新前)されるのでモーダルを再度開いても更新が反映されてない(このときにはdb更新されているが))